...
if [ $1 = '--help' ];then
	echo 'Commond for example:./service-deploy.sh <managed|unmanaged> <deploy|offline> [ServiceName].';
	exit 0;
fi

# 判断类型是否为托管服务，指定配置文件路径
if [ $TYPE = 'managed' ];then

	BASE_CONF_PATH=~/docker_deploy/conf/$TYPE/$SERVICE_NAME
